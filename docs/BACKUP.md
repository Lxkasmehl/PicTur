...
3. **After deployment:**
   ...
   - **Cron (one job for everything):** Prefer **`scripts/daily-backup.sh`**: it runs `python -m backup.run` (Sheets, including Primary ID sync then CSV/JSON), then `backfill_folder_names.py --apply` (renames on-disk turtle folders to match the just-exported Sheets state — gender corrections, juvenile→adult bio-ID changes, primary-ID rehoming), then `backup-backend-data.sh` (images), and finally restarts the backend container **only when the backfill actually applied changes** (so the VRAM cache reloads with the new file paths). Add **one** line with **`crontab -e`**. Do **not** paste the whole line into an interactive shell—the first five fields (`0 2 * * *`) are the schedule; the shell would try to run `0` as a command and print `command not found`.
   - Use an **absolute** path for `COMPOSE_DIR` (`~` often does not expand under cron). The recommended schedule is **02:00 server time** to minimise overlap with admin work (admin pages will get a 5-minute pre-window countdown overlay in a follow-up). Example (adjust paths and log location; `bash` avoids needing the execute bit on the scripts):
     ```cron
     0 2 * * * COMPOSE_DIR=/home/lukas/PicTur/TurtleTracker BACKUP_OUTPUT_DIR=/home/lukas/PicTur/TurtleTracker/backups /usr/bin/bash /home/lukas/PicTur/TurtleTracker/scripts/daily-backup.sh >> /home/lukas/pictur-backup.log 2>&1
     ```
   ...
   - **Sheets-only cron** (if you split jobs):
     ```cron
     0 2 * * * cd /home/lukas/PicTur/TurtleTracker && /usr/bin/docker compose exec -T backend python -m backup.run >> /home/lukas/pictur-backup.log 2>&1
     ```
